...
A Product Manager's portfolio project by [Christin Thomas](https://www.linkedin.com/in/0chris). Built to ship the unglamorous parts of agentic AI for real — evaluation, truth-grounding, model selection, reliability — and to prove the platform thesis by reusing the same orchestration layer for three unrelated products.

> Updated July 2026. The code is private; this page is a living overview of what's built and where the interesting decisions were.

---

## TL;DR

- **Three unrelated products on one runtime.** **Job Hunt** (3 agents covering a 5-stage application pipeline), **Study / Exam Hub** (5 agents — 4 student-facing plus a background Curator — an exam-focused, NotebookLM-style tutor with FSRS-6 spaced repetition, a grounding verifier, and full timed 3-section mock exams with pause), and **Product Studio** (a 7-agent software team that takes one idea to a shipped, working web app — with browser-based QA that actually runs what was built). Same orchestration layer, same eval harness, same guardrails — different domains. **Adding a module means new agents, rubrics, and tables — not a new scheduler.** That reuse is the whole point: it's a platform, not a one-off demo.
- **Built for trust, not demos.** A deterministic truth-grounding verifier blocks fabricated claims before they leave the app — on the screenshot below it clears 42 of 43 claims and correctly classifies the 43rd as an *honest gap*, not a lie. An evaluation harness scores every agent against per-role rubrics, with separate **graded** (0–100) and **guardrail** (pass/fail) dimensions and N-iteration averaging so a lucky single run can't mask flaky behaviour.
- **Model choice is an experiment, not a vibe.** A **Model A/B Lab** runs N `{provider, model}` candidates head-to-head against the incumbent on a fixed rubric, with a coverage-aware fairness contract, per-model cost + latency folded into the verdict, and a one-click apply/revert of the winner. The first comparison kept Researcher on MiniMax-M2.7 over Hermes-3-405B on a measured 15-point edge with no guardrail regression.
- **Hardened the way real products are.** After the first feature push I paused and gated new work behind a multi-sprint stabilization plan, then kept the discipline: **173 issues tracked across 8 subsystems, 172 Fixed, every Critical- and High-severity defect closed** (1 Critical / 43 High / 86 Medium / 43 Low; the single remaining item is an *Analysed* Medium enhancement, ISS-005). Atomic counters, optimistic concurrency, per-company FIFO locks, SSRF guards, recovery hygiene, an OS-level timeout backstop that survives a starved event loop, and an observability layer over an agent runtime I don't own.
...
flowchart TB
    ...
    Orchestrator["Orchestration layer · heartbeat scheduler + watchdog + recovery gates"]
    ...
    subgraph Studio ["Product Studio · 7-agent software team"]
        direction LR
        PM["PM"] --> Design["Tech Lead ∥ UI Designer"]
        Design --> Build["Frontend ∥ Backend"]
        Build --> QA["QA (runs the app)"]
    end
...
Fluid is a **two-layer platform**: an orchestration engine underneath (a heartbeat scheduler that wakes agents, an OS-level watchdog that catches them when they hang, three-point recovery gates that bring them back, and a Hermes Bridge telemetry seam) and a **module-agnostic UI shell** on top that now runs three unrelated application modules. Reliability, evaluation, and truth-grounding are platform properties every module inherits for free.

Two distinct execution paths sit underneath the same shell, which was a real design choice rather than an accident:

- **Async / heartbeat path** — Job Hunt, Study content import and Deep Dive, the Study Curator, and all of Product Studio. Work becomes a tracked issue; a heartbeat scheduler wakes the right agent; a bridge service lenient-parses the agent's output back into domain tables. This is where the truth verifier runs.
...
Keeping these explicitly separate — rather than forcing one path to serve both — was the architectural decision that made Study tractable on the Job Hunt shell. Product Studio then reused the generic issue/wakeup path with **zero new scheduler machinery**.
...
A direct consequence: when I added the Study module — and later a 7-agent Product Studio — "what roles, with what boundaries" was the design question I already knew how to answer.
...
Most AI products are static — each interaction is independent. Fluid captures every score override, rationale rewrite, star rating, and close reason as a timeline event. A pattern aggregator computes lift-based keyword discriminators ("opportunities I pursue mention *models / workflow / education* several-fold more often than ones I reject"; "consulting roles get close-rejected the large majority of the time"; soft deal-breaker keywords I downgrade) and produces a structured user fingerprint that gets injected into Scout's match-scoring prompt and every downstream agent's task prompt.
...
### 6. Module abstraction proven three times

The Study module shipped in a focused week and was the first test of whether the platform thesis was real or marketing. Shared with Job Hunt: UI shell, sidebar, Home, agent runtime, profile/fact infrastructure, eval harness, meeting synthesizer, reliability primitives, on-disk Hermes profile management. Module-specific: schema, prompts, personas, pages, FSRS engine, automations.

The Job Hunt-only Home was rewritten into a module-agnostic, content-forward shell (shared activity feed, per-module dashboard cards) so no module is privileged. Study v2 went further — added a fifth "Curator" agent that ingests user material and open-web sources into a per-topic knowledge base, plus a guided on-ramp UI for students starting from zero, and reframed the module toward exam prep (GMAT-focused, timed 3-section mocks with pause). The third proof was the most demanding: **Product Studio** (below) put a 7-agent software team on the same runtime — same issue/wakeup pattern, no new scheduler machinery — and it held.
...
This is the section most "AI agents" projects skip. The agents are the surface area where everything else lives — eval, truth-grounding, supervision — so the persona-level decisions are the product decisions. **15 specialized agents run across the three modules.**
...
**Per-agent model selection is a data-model concern, not a code change.** The Job Hunt agents (Scout, Researcher, Writer) and the background workers (Orchestrator, Curator, Note Taker) run on `MiniMax-M2.7` via the local `minimax` provider. Study's interactive agents each run a model picked for their job — **Tutor** on Nemotron-3-Super-120B, **Quiz Master** on Arcee Trinity Large Thinking, **Study Planner** on DeepSeek-V4-Flash (all via OpenRouter's free tier). The `adapterConfig` JSON column carries `{ model, provider }` per agent, and the eval harness records the model used on every run.
...
The **Model A/B Lab** is how I decide each split with evidence instead of intuition. The first comparison — Researcher, MiniMax-M2.7 vs Hermes-3-405B — kept the incumbent on a 15-point graded edge with no guardrail regression and far lower latency.
...
### Study / Exam Hub agents (5)

Five agents — four student-facing and one background worker — power an exam-focused, NotebookLM-style tutor (currently GMAT), with a grounding verifier over ingested material and full timed 3-section mock exams with pause. The split between **synchronous** agents (Tutor, Quiz Master in chat) and **asynchronous** agents (Study Planner for Deep Dive synthesis, Note Taker for card generation, Curator for content build-out) is structural — sync agents prioritise latency, async agents prioritise output discipline.
...
4. **A real designer.** A seventh agent, the UI Designer, joined in Sprint 4 and runs in parallel with the Tech Lead: screen inventory, empty/loading/error states, locked design tokens, and an actual HTML mockup the Frontend Engineer builds against — and QA audits the shipped UI against those locked tokens.
...
After the first big feature push, dogfooding surfaced a class of failures that no amount of further features would have fixed. I gated new feature work behind a hardening plan and kept the discipline as the project grew to three modules: **173 issues tracked across 8 subsystems, 172 Fixed, every Critical- and High-severity defect closed** (1 Critical / 43 High / 86 Medium / 43 Low; the one remaining item is an *Analysed* Medium enhancement, ISS-005). Each issue lives in a per-issue markdown file, indexed by a generated `ISSUE_LOG.md` (`pnpm bugs:build`), and every fix is a small, scoped commit with an issue ID — the backlog itself is the artifact.

The five most instructive incidents — the kind of failure mode you only learn by running real agents on a real machine:

**1. Laptop-killing 30+GB memory crash (Lark import wedge).** A diagnostic helper did an actual `import lark_oapi` just to *test availability*. `lark_oapi` is a large SDK; importing it compiles bytecode for the whole API. Under memory pressure a 2-second check became a 15-minute swap-thrash that wedged agent init. Fix: `importlib.util.find_spec()` (no import, ~50ms) plus a concurrency cap with overflow queued and drained as slots free. Capability *probes* are not free.
...
- **Fewer features, deeper quality.** Fluid has Chat, Meetings, Job Hunt, Study, Product Studio, an Electron app, a Chrome extension, a full design system. If I started over I'd build only Job Hunt with exceptional agent quality and ship the second module after.

---

## Also in the repo (directional, not shipped as product)

Two things live in the codebase that aren't full modules — included here for honesty about where the platform is heading, not counted in the numbers above:

- **The Office** — a 2.5D agent-floor visualization (`officeSim.ts`) that renders agents as characters moving around a workspace, an experiment in making the orchestration layer legible at a glance.
- **Freelancer** — a directional spike, reframed mid-design from an auto-bidder into a *demo-first proposal studio* (build a small proof, then pitch it). Design-only; not shipped.
...
| Evaluation suite | Study / Exam Hub module |
|---|---|
...
| Backend | Node.js, Express, Server-Sent Events; PostgreSQL + Drizzle ORM with explicit migrations (`0090`–`0125`) |
...
| Study-specific | FSRS-6 spaced repetition (`ts-fsrs`) with per-student weight optimization (Python subprocess); grounding verifier over ingested material; timed 3-section mock exams with pause; sync sessions over an awaited Hermes turn through a concurrency semaphore |
| Product Studio-specific | Per-product workspace with canonical `SPEC.md` / `DESIGN.md` / `DESIGN_UX.md`; automated boot check (install + wait-for-port) gating every build; one-click local preview deploy; managed sandboxed headless Chromium (CDP `:9224`) for browser-based QA; studio-wide `lessons.md` cross-run memory; per-product `studioWeight` (full/quick) |
| Evaluation | Rubric-driven harness (graded 0–100 vs. pass/fail guardrail dimensions, N-iteration averaging); Model A/B Lab with a coverage-aware fairness contract; promptfoo for prompt-level checks |
...
I'm **Christin Thomas** — a Product Manager focused on AI products. Fluid is a personal portfolio project: the code is private; this page is a living overview of what's built and how, and where the interesting decisions were. It's deliberately a *platform* decision, not a feature demo — one orchestration layer, three unrelated products on top of it, and the unglamorous reliability, evaluation, and truth-grounding work that makes agents trustworthy.
